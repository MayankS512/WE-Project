...
const Dashboard = ({ server }) => {
  ...
  return (
    <div  className='flex flex-col w-full h-full col-span-3 overflow-auto custom md:col-span-2 lg:col-span-3 xl:col-span-4'>
      ...
      <div id='messages' className='flex flex-col-reverse flex-grow p-2 m-2 mb-0 overflow-auto text-white rounded-md bg-zinc-800'>
        {documents && documents.map(message => {
          return <Message key={message.id} from={message.from} at={message.createdAt} avatar={message.avatar}>{message.body}</Message>
        })}
      </div>
      <UserInput />
    </div>
